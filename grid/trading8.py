...
usdc_per_order = 25
buy_drop_pct = 0.006847
buy_pullback_pct = 0.000869
sell_gain_pct = 0.015324
sell_pullback_pct = 0.001094
fee_pct = 0.001
...
for i in range(1, len(df)):
    ...
    if should_buy:
        ...
        cov = len(positions)
        print(f"[{time}] ✅ Achat à {price:.3f} USDC (qty: {sol_qty:.4f}), pos: {cov}, fee: {fee}")
...
print(f"\n🔚 Profit total simulé : {profit:.2f} USDC, nb_trans = {nb_trans}, total_fee = {total_fee}")
